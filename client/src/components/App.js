...
import NewQuizForm from "./pages/Quiz-form/New-Quiz-form";
...
import TakeQuiz from "./pages/Quiz-details/Take-Quiz";
...
class App extends Component {
  ...
  handleToast = (visible, text, color) =>
    this.setState({ showToast: visible, toastText: text, toastColor: color });

  updateFavCourses = (item_id) => {
    if (this.state.loggedInUser) {
      const newList = [...this.state.loggedInUser.favCourses].some(
        (elm) => elm === item_id
      )
        ? [...this.state.loggedInUser.favCourses].filter(
            (elm) => elm !== item_id
          )
        : [...this.state.loggedInUser.favCourses, item_id];
      this.usersServices
  ...
  updateFavTeachers = (item_id) => {
    if (this.state.loggedInUser) {
      const newList = [...this.state.loggedInUser.favTeachers].some(
        (elm) => elm === item_id
      )
        ? [...this.state.loggedInUser.favTeachers].filter(
            (elm) => elm !== item_id
          )
        : [...this.state.loggedInUser.favTeachers, item_id];
      this.usersServices
          ...
          <AnimatePresence>
            <Switch>
              <Route
                ...
                path="/"
                render={(props) => (
                  <Home {...props} handleToast={this.handleToast} />
                )}
              />
              <Route
                exact
              ...
              />
              <Route
                path="/profile-teacher/create-quiz"
                render={(props) =>
                  this.state.teacher ? (
                    <NewQuizForm
                      {...props}
