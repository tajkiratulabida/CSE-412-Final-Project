...
router.post(
  ...
  [
    check("name")
      ...
      .custom((value) => {
        return Teacher.findOne({ name: value }).then((teacher) => {
          if (teacher) {
            return Promise.reject("The name already exists, try another one");
          }
        });
      }),
    ...
    check("surname")
      .isLength({ min: 3 })
      .withMessage("Surname should have min 3 characters."),

    check("jobOccupation")
      .isLength({ min: 10 })
      .withMessage("Write a few words that describe your profession"),

    check("description")
      .isLength({ min: 10 })
      .withMessage("Description should have min 3 characters."),
  ],
  (req, res) => {
...
router.delete(
  ...
  (req, res) => {
    ...
    Course.deleteMany({ owner: teacher_Id })
